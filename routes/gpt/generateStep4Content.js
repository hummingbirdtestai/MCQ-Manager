...
async function generateStep4Content(req, res) {
  ...
  try {
    ...
    // 3. Insert or merge with Supabase
    ...
    if (fetchError) {
      ...
      return res.status(500).json({ error: '❌ Failed to fetch existing topic uploads' });
    }
    ...
    if (existing && existing.length > 0) {
      ...
      if (updateError) {
        ...
        return res.status(500).json({ error: '❌ Failed to update topic uploads' });
      }
    } else {
      ...
      if (insertError) {
        ...
        return res.status(500).json({ error: '❌ Failed to insert topic uploads' });
      }
    }

    return res.status(200).json({ success: true, message: '✅ Step 4 content generated and stored', step: 4, inserted: parsed });
  } catch (err) {
